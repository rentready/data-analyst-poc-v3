...
class CosmosDBKnowledgeBaseSearchTool:
    ...
    async def search_and_format(
        ...
        """
        try:
            logger.info(f'≡ƒöì Cosmos DB KB searching for: "{query}" (top_k={top_k}, type={search_type})')
            ...
            if not results:
                logger.info('Γ¥î No results found in Cosmos DB')
            ...
            logger.info(f'Γ£à Formatted {len(results)} Cosmos DB results ({len(formatted_text)} chars)')
            return formatted_text
            
        except Exception as e:
            logger.error(f'Γ¥î Cosmos DB KB search error: {e}', exc_info=True)
    ...
    def _format_captions(captions: List[Any]) -> str:
        ...
        for caption in captions[:MAX_CAPTIONS_PER_RESULT]:
            ...
            if caption_text:
                caption_lines.append(f'  ΓÇó {caption_text}')
